...
TEST_F(ModulationMatrixTest, InitDoesNotCrash) {
    ...
    EXPECT_EQ(matrix_.GetDestination(ModSource::Lfo2), ModDestination::Color);
    EXPECT_EQ(matrix_.GetAmount(ModSource::Lfo1), 0);
}

TEST_F(ModulationMatrixTest, SetDestinationWorks) {
    matrix_.SetDestination(ModSource::Lfo1, ModDestination::Color);
    EXPECT_EQ(matrix_.GetDestination(ModSource::Lfo1), ModDestination::Color);
...
TEST_F(ModulationMatrixTest, EnvelopeTriggerWorks) {
    matrix_.SetDestination(ModSource::Env1, ModDestination::Color);
    ...
    matrix_.Process(kSampleRate, 64);
    float mod_before = matrix_.GetModulation(ModDestination::Color);
    ...
    float mod_after = matrix_.GetModulation(ModDestination::Color);
